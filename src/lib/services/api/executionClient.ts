...
export class ExecutionClient {
  ...
  /**
   * Legacy execute endpoint that accepts a flat list of tasks
   */
  ...
  /**
   * Execute a full graph payload of nodes and edges as required by the backend
   */
  async executeGraph(payload: { nodes: Array<{ id: string; type: string; inputs?: Record<string, any>; data?: Record<string, any> }> }): Promise<ExecuteResponse> {
    const url = `${this.baseUrl}/execute`;
    const headers: HeadersInit = { 'Content-Type': 'application/json' };
